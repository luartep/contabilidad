...
export default function F29Page() {
  ...
  async function handleAgregarDoc(e: React.FormEvent) {
    ...
    const esAfecta = !form.tipo_documento.includes("exenta") && form.tipo_documento !== "nota_credito_exenta";
    const iva = form.monto_iva !== "" ? Number(form.monto_iva) : (esAfecta ? Math.round(neto * 0.19) : 0);
    const exento = Number(form.monto_exento) || 0;

    const res = await fetch("/api/f29/documentos", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        empresa_id: id, periodo,
        ...form,
        monto_neto: neto,
        monto_iva: iva,
        monto_exento: exento,
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-4">
      ...
      <div className="flex items-start justify-between">
        ...
        {periodoF29?.estado === "presentado" && (
          <span className="text-xs bg-green-100 text-green-700 rounded-full px-3 py-1.5 mt-3">
            ✓ Presentado
          </span>
        )}
      </div>
        ...
        <div className="flex items-end gap-4 flex-wrap">
          ...
          <div className="space-y-1">
            ...
            <input type="number" min="0"
              ...
              value={remAnt}
              onChange={(e) => setRemAnt(e.target.value)}
              placeholder="0" />
          </div>
        </div>
        {impuestoUnico > 0 && (
          <p className="mt-3 text-xs text-teal-700 bg-teal-50 rounded-lg px-3 py-2">
            💡 Impuesto único de trabajadores del período: <strong>{clp(impuestoUnico)}</strong> (traído desde Remuneraciones)
          </p>
        )}
      </div>

      {/* Tabs ventas / compras / honorarios */}
        ...
        <div className="p-5">
          ...
          {!showForm ? (
            <button onClick={() => { setShowForm(true); setForm({ ...docDefault, tipo: tab === "ventas" ? "venta" : tab === "compras" ? "compra" : "honorario_recibido" }); }}
              ...
              + Agregar {tab === "ventas" ? "venta" : tab === "compras" ? "compra" : "honorario recibido"}
            </button>
          ) : (
            <form onSubmit={handleAgregarDoc} className="border border-slate-200 rounded-xl p-4 mb-4 space-y-3 bg-slate-50">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                    value={form.folio} onChange={(e) => setForm({ ...form, folio: e.target.value })} placeholder="123" />
                </div>
                ...
                {tab !== "honorarios" ? (
                  <>
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-medium text-slate-600">IVA ($) — se calcula al 19% si dejas vacío</label>
                      <input type="number" min="0" className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                        value={form.monto_iva} onChange={(e) => setForm({ ...form, monto_iva: e.target.value })} placeholder="Auto (19%)" />
                    </div>
                    <div className="space-y-1">
                  ...
                  <>
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-medium text-slate-600">Retención (13.75% por defecto)</label>
                      <input type="number" min="0" className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
          ...
          {tabDocs.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-50">
                  {tabDocs.map((d: any) => (
                    <tr key={d.id} className={d.es_nota_credito ? "text-red-600" : ""}>
                      ...
                      <td className="py-2 pr-3 text-right font-medium">{clp(d.monto_neto)}</td>
                      ...
                      <td className="py-2">
                        <button onClick={() => handleEliminar(d.id)}
                          className="text-xs text-red-400 hover:text-red-700 transition">✕</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
                <tfoot className="border-t border-slate-200">
                  <tr className="font-semibold text-slate-700">
                    ...
                    <td className="pt-2 text-right">{clp(tabDocs.reduce((s: number, d: any) => s + Number(d.monto_neto), 0))}</td>
                    <td className="pt-2 text-right">{clp(tabDocs.reduce((s: number, d: any) => s + Number(d.monto_iva || d.retencion_honorario), 0))}</td>
                    <td className="pt-2 text-right">{clp(tabDocs.reduce((s: number, d: any) => s + Number(d.monto_total), 0))}</td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
            ...
            <p className="text-sm text-slate-400 text-center py-4">Sin documentos en esta categoría</p>
          )}
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        ...
        {resultado ? (
          <>
            <div className="grid grid-cols-2 gap-x-8 gap-y-0 text-sm">
              ...
              <div className="space-y-1 border-r border-slate-100 pr-8">
                <p className="text-xs font-semibold text-slate-400 uppercase mb-2">Débito Fiscal (Ventas)</p>
                ...
                {resultado.ppm_monto > 0 && (
                  <div className="flex justify-between py-1 border-b border-slate-50">
                    <span className="text-slate-500">PPM ({empresa?.tasa_ppm}% sobre {clp(resultado.ppm_base)})</span>
              ...
              <div className="space-y-1 pl-8">
                <p className="text-xs font-semibold text-slate-400 uppercase mb-2">Crédito Fiscal (Compras)</p>
                <div className="flex justify-between py-1 border-b border-slate-50">
                ...
                {resultado.remanente_credito > 0 && (
                  <div className="flex justify-between py-1 border-b border-slate-50 text-blue-700">
                    <span>Remanente para próximo mes</span>
            ...
            {/* Total a pagar */}
            <div className={`mt-5 rounded-xl p-4 flex items-center justify-between ${resultado.total_a_pagar > 0 ? "bg-teal-700 text-white" : "bg-slate-100 text-slate-700"}`}>
              <div>
                ...
                {resultado.total_a_pagar === 0 && resultado.remanente_credito > 0 && (
                  <p className="text-xs text-blue-600 mt-0.5">Crédito fiscal mayor al débito — sin pago este mes</p>
                )}
              </div>
          ...
          <p className="text-sm text-slate-400 text-center py-6">
            Agrega los documentos de ventas y compras del período, luego presiona "Calcular F29".
          </p>
        )}
      </div>
